...
/** @var \Illuminate\Database\Eloquent\Collection|\App\Models\Horario[] $horarios */
?>
...
<section class="container admin">
    ...
    <h1 class="mt-3">Editar: {{ $curso->nombre }}</h1>
    <p>Editá los datos del curso en el siguiente formulario. Para guardar los cambios hacé click en "Actualizar".</p>
    @if($errors->any())
    ...
    @endif
    <form action="{{ route('admin.cursos.editar', ['id' => $curso->curso_id]) }}" method="post" enctype="multipart/form-data" class="row form-admin">
        @csrf
            ...
            >
                @foreach ($duraciones as $duracion)
                    <option 
                        ...
                        @selected($duracion->duracion_id == old('duracion_id', $curso->duracion_id))
                    >
                    {{ $duracion->duracion }}
                    </option>
                @endforeach
            ...
            @foreach ($modalidades as $modalidad)
                <div class="form-check form-check-inline">
                    <input 
                        ...
                        @checked(in_array($modalidad->modalidad_id, old('modalidades', $curso->getModalidadesId())))
                    >
            ...
            @foreach ($horarios as $horario)
                <div class="form-check form-check-inline">
                    <input 
                        ...
                        @checked(in_array($horario->horario_id, old('horarios', $curso->getHorariosId())))
                    >
        ...
        <fieldset class="mb-3 col-lg-10">
            ...
            <div class="form-check">
                <input 
                    ...
                    @checked(in_array($objetivo->objetivo_id, old('objetivos', $curso->getObjetivosId())))
                >
                <label for="objetivo-{{ $objetivo->objetivo_id }}" class="form-check-label">{{ $objetivo->objetivo }}</label>
            </div>
            @endforeach
        </fieldset>
        <div class="info-img col-lg-10">
        ...
        <div class="info-img col-lg-10">
            @if($curso->img != null && Storage::disk('public')->has('img/' . $curso->img))
              <p>Imagen Actual:</p>
              <img src="{{ Storage::disk('public')->url('img/' . $curso->img) }}" alt="{{ $curso->img_descripcion }}">
              <p class="visually-hidden">Hay una imagen incluída.</p>
              <p>Si querés mantener esta foto, simplemente dejá el campo para subir imagen vacío.</p>
            @else
              <p>Actualmente no hay ninguna imagen cargada.</p>
        ...
        <div class="mb-3 col-lg-5">
            ...
            <input 
                ...
                class="form-control"
                value="{{ old('img_descripcion', $curso->img_descripcion) }}"
            >
        </div>
